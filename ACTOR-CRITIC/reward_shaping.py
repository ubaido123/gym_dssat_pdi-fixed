...
# ── Shaping annealing schedule ──────────────────────────────────────────
# Shaping bonuses (LAI growth/checkpoint/water-saving terms) are useful
# early -- they give the agent a dense learning signal when the raw
# profit-linked base_norm reward alone is too sparse/noisy to learn from.
# But left at full weight for the whole run, they compete with base_norm
# indefinitely, and the agent ends up partially optimizing "hit LAI
# targets" rather than "maximize profit" -- unlike REINFORCE, whose
# reward IS profit throughout. Anneal shaping weight down linearly so the
# policy increasingly optimizes raw profit as training progresses.
SHAPING_ANNEAL_START_EPISODE = 500    # full shaping weight before this
SHAPING_ANNEAL_END_EPISODE   = 3500   # floor shaping weight from this point on
SHAPING_MIN_WEIGHT           = 0.15   # never fully zero -- keeps some
                                       # dense signal so late-training
                                       # exploration doesn't go fully blind


def shaping_weight_for_episode(episode):
    if episode <= SHAPING_ANNEAL_START_EPISODE:
        return 1.0
    if episode >= SHAPING_ANNEAL_END_EPISODE:
        return SHAPING_MIN_WEIGHT
    span = SHAPING_ANNEAL_END_EPISODE - SHAPING_ANNEAL_START_EPISODE
    progress = (episode - SHAPING_ANNEAL_START_EPISODE) / span
    return 1.0 - progress * (1.0 - SHAPING_MIN_WEIGHT)
...
class SeasonalRewardShaper:
    ...
    def __init__(self, crop_name: str, verbose: bool = True):
        ...
        self._shaping_weight = shaping_weight_for_episode(0)
    ...
    def reset(self, crop_name: str, episode: int = 0):
        ...
        self._prev_xlai  = 0.0
        self._prev_topwt = 0.0
        self._shaping_weight = shaping_weight_for_episode(episode)
    ...
    def step(self, obs_dict, raw_reward, done,
        ...
        for cp_num, cp_dap in enumerate(CHECKPOINT_DAPS, start=1):
            if dap >= cp_dap and cp_dap not in self._triggered:
                ...
                if self.verbose:
                    ...
                    reward_pct = (base_norm + self._shaping_weight * (shaping + checkpoint_bonus)) * 100.0
        ...
        return base_norm + self._shaping_weight * (shaping + checkpoint_bonus)